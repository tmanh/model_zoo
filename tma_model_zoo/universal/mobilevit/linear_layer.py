...
class LinearLayer(BaseLayer):
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(in_features={self.in_features}, out_features={self.out_features}, bias={self.bias is not None}, channel_first={self.channel_first})"
...
class GroupLinear(BaseLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if in_features % n_groups != 0:
            logger.error(f"Input dimensions ({in_features}) must be divisible by n_groups ({n_groups})")

        if out_features % n_groups != 0:
            logger.error(f"Output dimensions ({out_features}) must be divisible by n_groups ({n_groups})")
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(in_features={self.in_features}, out_features={self.out_features}, groups={self.n_groups}, bias={self.bias is not None}, shuffle={self.feature_shuffle})"
